...
void initperception()  {
	(void) Py_InitModule("perception", PerceptionMethods);
	import_array();  // Must be present for NumPy.  Called first after above line.
}

double activation(double x, double y, double fx, double fy, double a, double s) {
	return a * exp( -( ((fx-x)*(fx-x) + (fy-y)*(fy-y))) / (2*s*s) );
}

static PyObject * pmap(PyObject *self, PyObject *args) {

	PyArrayObject *x, *y, *objX, *objY, *objScore, *matout;
	double *X, *Y, *OBJX, *OBJY, *OBJSCORE, s, **MATOUT;
	int width, height, dims[2], row, col, n, i;
	...
	if (!PyArg_ParseTuple(args, "O!O!O!O!O!d",
			...
			&PyArray_Type, &objX,
			&PyArray_Type, &objY,
			&PyArray_Type, &objScore,
			&s)) return NULL;
	if (NULL == x) return NULL;
	if (NULL == y) return NULL;
	if (NULL == objX) return NULL;
	if (NULL == objY) return NULL;
	if (NULL == objScore) return NULL;

	width = dims[0] = x->dimensions[0];
	height = dims[1] = y->dimensions[0];
	n = objX->dimensions[0];

	X = pyvector_to_Carrayptrs(x);
	Y = pyvector_to_Carrayptrs(y);
	OBJX = pyvector_to_Carrayptrs(objX);
	OBJY = pyvector_to_Carrayptrs(objY);
	OBJSCORE = pyvector_to_Carrayptrs(objScore);

	matout = (PyArrayObject *) PyArray_FromDims(2,dims,NPY_DOUBLE);
	MATOUT = pymatrix_to_Carrayptrs(matout);

	for (row = 0; row < height; row++) {
		for (col = 0; col < width; col++) {
			MATOUT[col][row] = 0;
			for (i = 0; i < n; i++) {
				MATOUT[col][row] += activation(X[col],Y[row],OBJX[i],OBJY[i],OBJSCORE[i],s);
			}
		}
	}

	free_Carrayptrs(MATOUT);
	return PyArray_Return(matout);
...
/*static PyObject * maxima(PyObject *self, PyObject *args) {
    ...
    int turnpoints[width*height];
    for (row = 0; row < height; row++)
		for (col = 0; col < width; col++)
			turnpoints[col*height + row] = 0;

    double prev,cur,next;
    for (row = 0; row < height; row++) {
    	cur = matrix[0*height + row];
		next = matrix[1*height + row];
		if (cur>next)
			turnpoints[0*height + row] += 1;
		else if (cur<next)
			turnpoints[0*height + row] += -1;
		for (col = 1; col < width-1; col++) {
			prev = matrix[(col-1)*height + row];
			cur = matrix[col*height + row];
			next = matrix[(col+1)*height + row];
			if (prev<cur && cur>next)
				turnpoints[col*height + row] += 1;
    ...
    for (col = 0; col < width; col++) {
    	cur = matrix[col*height + 0];
		next = matrix[col*height + 1];
		if (cur>next)
			turnpoints[col*height + 0] += 1;
		else if (cur<next)
			turnpoints[col*height + 0] += -1;
    	for (row = 1; row < height-1; row++) {
			prev = matrix[col*height + (row-1)];
			cur = matrix[col*height + row];
			next = matrix[col*height + (row+1)];
			if (prev<cur && cur>next)
				turnpoints[col*height + row] += 1;
			else if (prev>cur && cur<next)
				turnpoints[col*height + row] += -1;
    ...
    for (row = 0; row < height; row++) {
		for (col = 0; col < width; col++) {
			if (turnpoints[col*height + row] == 2)
				rans[col*height + row] = matrix[col*height + row];
			else
				rans[col*height + row] = NA_REAL;
		}
	}
